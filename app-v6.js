...
const phases = {
  ...
  2: { label: "2. Hafta · Temel", note: "Ana hareket kalıplarını koru. Göğüs, kol ve karın hedefleri için temel hacmi kuruyoruz." },
  3: { label: "3. Hafta · Hacim", note: "Ana hareketlerde hacmi artırıyoruz. Karın artık haftada 9 direkt set alıyor." },
  4: { label: "4. Hafta · Progresyon", note: "Üst tekrar sınırı temiz ve kontrollü geliyorsa ağırlığı küçük miktarda artır." },
  5: { label: "5+ Hafta · Kas Gelişimi", note: "Ana hareketleri 6–8 hafta sabit tutup kademeli ağırlık veya tekrar artır. Göğüs ve karın yaklaşık 9–10 direkt set/hafta alır." }
};

const dayInfo = {
  A: "Göğüs · sırt · squat · hinge · karın · biceps",
  B: "Üst göğüs · sırt · tek bacak · omuz · karın · triceps",
  C: "Göğüs · sırt · hinge · karın · biceps"
};
...
const warmup = [
  ...
  { id:"deadbug", name:"Dead Bug", meta:"Core kontrolü · 6/yan", sets:1, reps:"6/yan", weighted:false, lordosis:"Bel boşluğunun artmasına izin verme; kontrol kaybolursa hareket mesafesini kısalt.", cues:["Kalça ve diz yaklaşık 90°.","Karşı kol ve bacağı yavaş uzat.","Nefes verirken karın duvarını aktif tut."] },
...
const week1 = {
  A: [
    ...
    {id:"deadbug",name:"Dead Bug",muscles:"Karın · core kontrolü",reps:"6/yan",sets:2,weighted:false,cue:"Karşı kol ve bacağı yavaş uzat.",lordosis:"Bel boşluğu artarsa hareket mesafesini kısalt."}
  ],
  B: [
    ...
    {id:"sideplank",name:"Side Plank",muscles:"Karın · oblik · core kontrolü",reps:"20–30 sn/yan",sets:2,weighted:false,cue:"Baş–omuz–kalça aynı çizgide.",lordosis:"Kalçayı düşürme ve gövdeyi çevirmeden nefes almaya devam et."}
  ],
  C: [
    ...
    {id:"reversecrunch",name:"Reverse Crunch",muscles:"Karın · pelvis kontrolü",reps:"8–10",sets:2,weighted:false,cue:"Kuyruk sokumunu kontrollü yerden kaldır.",lordosis:"Momentum yerine kontrollü posterior pelvic tilt üret."},
...
const baseProgram = {
  A: [
    ...
    {id:"crunch",name:"Crunch / Dumbbell Crunch",muscles:"Karın · rectus abdominis",reps:"10–15",weighted:true,cue:"Kaburgaları pelvise doğru yaklaştır; boynundan çekme. 15 tekrar rahat gelince göğüste hafif dumbbell kullan.",lordosis:"Belini zemine zorla bastırma; hareketi kontrollü gövde fleksiyonu ile yap."},
  ...
  B: [
    ...
    {id:"shoulder",name:"Seated Dumbbell Shoulder Press",muscles:"Omuz · triceps",reps:"8–12",weighted:true,cue:"Sehpaya otur; dumbbell'ları kontrollü press et.",lordosis:"Kaburgaları aşağıda tut; beli aşırı çukurlaştırma."},
    {id:"reversecrunch",name:"Reverse Crunch",muscles:"Karın · rectus abdominis",reps:"10–15",weighted:false,cue:"Dizleri göğse savurma; kuyruk sokumunu kontrollü kaldır.",lordosis:"Momentum yerine kontrollü posterior pelvic tilt üret."},
  ...
  C: [
    ...
    {id:"crunch",name:"Crunch / Dumbbell Crunch",muscles:"Karın · rectus abdominis",reps:"10–15",weighted:true,cue:"Kaburgaları pelvise doğru yaklaştır; boynundan çekme. 15 tekrar rahat gelince göğüste hafif dumbbell kullan.",lordosis:"Belini zemine zorla bastırma; kontrollü gövde fleksiyonu kullan."},
...
const tenorPosts = {
  ...
  crunch:{id:"27383226",url:"https://tenor.com/view/abs-crunch-gif-27383226",ratio:"1.79"}
};
...
function programForWeek(week){
  ...
  Object.entries(baseProgram).forEach(([day,rows])=>{
    program[day]=rows.map((r,i)=>({...r,sets:(+week===5 && day==="A" && i===0)?4:sets}));
  });
  if(+week===2){
...
function volumeForWeek(week){
  const rows=Object.values(programForWeek(week)).flat();
  const sum=(fn)=>rows.filter(fn).reduce((a,r)=>a+(+r.sets||0),0);
  return { chest:sum(r=>/Göğüs/i.test(r.muscles)), abs:sum(r=>/Karın/i.test(r.muscles)), biceps:sum(r=>r.id==="curl"||r.id==="hammer"), triceps:sum(r=>r.id==="triceps") };
}
...
function latestLogForExercise(id){ return (state().logs||[]).find(x=>x.id===id && x.weight!==undefined && x.weight!==null && String(x.weight).trim()!==""); }

function renderExercise(ex,parent,scope,week){
  const tpl=q("#exerciseTemplate").content.cloneNode(true), card=tpl.querySelector(".exercise"), demo=tpl.querySelector(".demo"), fullScope=weekScope(week,scope);
  ...
  tpl.querySelector(".sets").value=ex.sets; tpl.querySelector(".reps").value=ex.reps; tpl.querySelector(".save-log").textContent="Hareketi kaydet";
  const weightInput=tpl.querySelector(".weight"), weightLabel=weightInput.closest("label"), prev=latestLogForExercise(ex.id);
  if(ex.weighted===false){ weightLabel.style.display="none"; }
  else if(prev){ weightInput.value=prev.weight; const note=document.createElement("div"); note.className="last-performance"; const d=new Date(prev.time); note.innerHTML=`<span>Son kayıt</span><b>${prev.weight} kg</b><small>${prev.sets} set · ${prev.reps} · ${d.toLocaleDateString("tr-TR")}</small><em>Ağırlık otomatik dolduruldu</em>`; tpl.querySelector(".logging").insertAdjacentElement("beforebegin",note); }
  const cues=tpl.querySelector(".cues"); [ex.cue,...(ex.cues||[])].filter(Boolean).forEach(c=>{const li=document.createElement("li");li.textContent=c;cues.appendChild(li);});
  ...
  tpl.querySelector(".demo-toggle").onclick=(e)=>{const wrap=e.currentTarget.nextElementSibling,willOpen=wrap.hidden;wrap.hidden=!wrap.hidden;if(willOpen)loadTenorDemo(demo,ex.id);e.currentTarget.textContent=wrap.hidden?"Hareketi göster":"Gösterimi kapat";};
  tpl.querySelector(".done").onchange=(e)=>{const st=state();st.done[fullScope+"::"+ex.id]=e.target.checked;saveState(st);updateStats();};
  tpl.querySelector(".save-log").onclick=(e)=>{const c=e.currentTarget.closest(".exercise"),st=state();st.logs.unshift({time:new Date().toISOString(),week,scope,id:ex.id,name:ex.name,sets:c.querySelector(".sets").value,reps:c.querySelector(".reps").value,weight:ex.weighted===false?"":c.querySelector(".weight").value});st.done[fullScope+"::"+ex.id]=true;saveState(st);c.querySelector(".done").checked=true;e.currentTarget.textContent="Kaydedildi ✓";setTimeout(()=>e.currentTarget.textContent="Hareketi kaydet",900);updateStats();renderHistory();};
...
function renderToday(){
  const el=q("#today"),st=state(),day=st.selectedDay,w=+st.week,program=programForWeek(w),vol=volumeForWeek(w);
  el.innerHTML=`${daySwitcher(day)}<div class="priority-strip"><b>Öncelik: Göğüs · Kol · Karın</b><span>Bu hafta: göğüs ${vol.chest} direkt set · karın ${vol.abs} direkt set${w>=3?` · biceps ${vol.biceps} direkt + row katkısı · triceps ${vol.triceps} direkt + press katkısı`:""}</span></div><div class="science-strip"><b>${phases[w].label}</b><span>${phases[w].note}</span></div><h2 class="section-title">Gün ${day}</h2><p class="section-note">Önce 6–8 dakikalık ısınmayı tamamla. Set sonunda formun bozulmadan birkaç tekrar daha çıkarabilecek pay bırak; tükenişe gitmek gerekmiyor.</p>`;
  bindDaySwitcher(el); program[day].forEach(ex=>renderExercise(ex,el,"day"+day,w));
}

function renderWarmup(){ const el=q("#warmup"),w=+state().week; el.innerHTML='<h2 class="section-title">6–8 dakikalık ısınma</h2><p class="section-note">Dead Bug ve Glute Bridge burada lumbopelvik kontrol için var. Karın kası geliştirme setleri ayrıca ana programda Crunch ve Reverse Crunch ile yapılır.</p>'; warmup.forEach(ex=>renderExercise(ex,el,"warmup",w)); }
...
function renderProgram(){
  const el=q("#program"),w=+state().week,program=programForWeek(w),vol=volumeForWeek(w),phaseNote=w===1?"İlk hafta karın egzersizleri önce kontrol odaklı: Dead Bug, Side Plank ve Reverse Crunch.":"Karın için iki ana hareketi bilinçli olarak tekrar ediyoruz: Crunch ve Reverse Crunch. Amaç her hafta yeni hareket aramak değil, bu hareketlerde tekrar veya direnç progresyonu sağlamaktır.";
  el.innerHTML=`<div class="science-card card"><h2>Göğüs · Kol · Karın öncelikli yapı</h2><p>${phaseNote}</p><div class="volume-grid"><div><small>Göğüs</small><b>${vol.chest}</b><span>direkt set/hafta</span></div><div><small>Karın</small><b>${vol.abs}</b><span>direkt set/hafta</span></div><div><small>Biceps</small><b>${vol.biceps}</b><span>direkt + row katkısı</span></div><div><small>Triceps</small><b>${vol.triceps}</b><span>direkt + press katkısı</span></div></div><p class="bodyfat-note"><b>Karın görünürlüğü:</b> karın kasını progresif dirençle büyütebiliriz; görünürlük aynı zamanda vücut yağ oranına bağlıdır.</p></div><h2 class="section-title">${phases[w].label}</h2><p class="section-note">${phases[w].note}</p>`;
  Object.entries(program).forEach(([k,rows])=>{const card=document.createElement("div");card.className="day-card card detailed-day";card.innerHTML=`<div class="day-card-head"><div><h3>Gün ${k}</h3><p>${dayInfo[k]}</p></div></div><div class="program-list">${rows.map(r=>`<div class="program-exercise"><div><b>${r.name}</b><small>${r.muscles}</small></div><strong>${r.sets} × ${r.reps}</strong></div>`).join("")}</div><button class="primary open-day" data-day="${k}">Gün ${k}'yı aç</button>`;el.appendChild(card);});
  el.querySelectorAll(".open-day").forEach(b=>b.onclick=()=>openWorkoutDay(b.dataset.day));
}

function renderHistory(){const el=q("#history"),logs=state().logs||[];el.innerHTML='<h2 class="section-title">Antrenman geçmişi</h2><p class="section-note">Aynı hareketi tekrar açtığında en son kullandığın ağırlık otomatik olarak doldurulur.</p>';if(!logs.length){el.insertAdjacentHTML("beforeend",'<div class="card empty">Henüz kayıt yok.</div>');return;}const box=document.createElement("div");box.className="card";box.style.padding="16px";logs.slice(0,100).forEach(x=>{const d=new Date(x.time),wk=x.week?`Hafta ${x.week} · `:"",day=x.scope&&x.scope.startsWith("day")?`Gün ${x.scope.replace("day","")} · `:"";box.insertAdjacentHTML("beforeend",`<div class="log-item"><b>${x.name}</b><small>${d.toLocaleString("tr-TR")} · ${wk}${day}${x.sets} set · ${x.reps}${x.weight?` · ${x.weight} kg`:""}</small></div>`);});el.appendChild(box);}
function updateStats(){const st=state(),w=+st.week,day=st.selectedDay,program=programForWeek(w),scope=weekScope(w,"day"+day),total=program[day].length,done=program[day].filter(r=>st.done[scope+"::"+r.id]).length;q("#todayDone").textContent=Math.round(done*100/total)+"%";let wd=0;["A","B","C"].forEach(k=>{const ds=weekScope(w,"day"+k);if(program[k].every(r=>st.done[ds+"::"+r.id]))wd++;});q("#weekDone").textContent=wd+"/3";q("#logCount").textContent=(st.logs||[]).length;}
